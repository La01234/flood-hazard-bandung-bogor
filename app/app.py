...
# Default nilai per kota
DEFAULTS = {
    "Bandung": {
        "elevation": 722.0, "slope": 3.4, "aspect": 165.0,
        "TWI": 4.5, "HAND": 5.0, "NDVI": 0.28,
        "MNDWI": -0.39, "NDBI": 0.07,
        "SAR_baseline": -5.1, "dist_river": 570.0
    },
    "Bogor": {
        "elevation": 255.0, "slope": 3.9, "aspect": 164.0,
        "TWI": 4.5, "HAND": 4.0, "NDVI": 0.38,
        "MNDWI": -0.38, "NDBI": 0.0,
        "SAR_baseline": -5.7, "dist_river": 445.0
    }
}
...
with tab1:
    ...
    if kota_peta == "Keduanya":
        ...
        with col1:
            ...
            if os.path.exists('outputs/flood_susceptibility_map_bandung.png'):
                ...
                st_folium(m_bdg, width=400, height=400)
        ...
        with col2:
            ...
            if os.path.exists('outputs/flood_susceptibility_map_bogor.png'):
                ...
                st_folium(m_bgr, width=400, height=400)
        ...
        # Tambah marker pusat kota
        folium.Marker(
            location=center,
            popup=f"Pusat Kota {kota_peta}",
        ...
        if os.path.exists(img_path):
            ...
            with col_img:
                st.markdown(f"#### Flood Susceptibility Map")
    ...
    col_l1.markdown("🟢 **Rendah** — Susceptibility < 25%")
    col_l2.markdown("🟡 **Sedang** — Susceptibility 25–50%")
    col_l3.markdown("🟠 **Tinggi** — Susceptibility 50–75%")
    col_l4.markdown("🔴 **Sangat Tinggi** — Susceptibility > 75%")

# ============================================================
# TAB 2 — KOMPARASI MODEL INTERAKTIF
# ============================================================
with tab2:
    ...
    col1.metric("AUC Bandung (RF)", "0.70", delta=None)
...
(Banjir)"]
        ...
        fig_bar.add_trace(go.Bar(
            name="Bandung (RF)",
            x=metrics, y=bandung_vals,
            marker_color="#185FA5",
            text=[f"{v:.2f}" for v in bandung_vals],
            textposition="outside"
        ))
        fig_bar.add_trace(go.Bar(
            name="Bogor (XGBoost)",
            x=metrics, y=bogor_vals,
            marker_color="#993C1D",
            text=[f"{v:.2f}" for v in bogor_vals],
            textposition="outside"
        ))
        fig_bar.update_layout(
            barmode="group",
            yaxis=dict(range=[0, 1], title="Nilai"),
    ...
    # Slider threshold eksplorasi
    st.markdown("#### Eksplorasi Threshold")
    st.markdown("Geser slider untuk melihat pengaruh threshold terhadap prediksi:")

    col_sl1, col_sl2 = st.columns(2)

    with col_sl1:
        st.markdown("**Bandung — Random Forest**")
        threshold_bdg = st.slider(
            "Threshold Bandung",
            min_value=0.1, max_value=0.9,
            value=float(round(rf_threshold, 2)),
            step=0.01,
            key="slider_bdg"
        )
        st.caption(f"Default optimal: {rf_threshold:.2f}")
        if threshold_bdg < rf_threshold:
            st.info("⬇️ Threshold lebih rendah = lebih banyak area terdeteksi banjir (recall naik, precision turun)")
        elif threshold_bdg > rf_threshold:
            st.info("⬆️ Threshold lebih tinggi = lebih sedikit area terdeteksi banjir (precision naik, recall turun)")
        else:
            st.success("✅ Menggunakan threshold optimal")

    with col_sl2:
        st.markdown("**Bogor — XGBoost**")
        threshold_bgr = st.slider(
            "Threshold Bogor",
            min_value=0.1, max_value=0.9,
            value=float(round(xgb_threshold, 2)),
            step=0.01,
            key="slider_bgr"
        )
...
# ============================================================
# TAB 3 — PREDIKSI TITIK INTERAKTIF
# ============================================================
with tab3:
    ...
    with col_default:
        st.markdown(" ")
        st.markdown(" ")
        if st.button("📍 Gunakan nilai rata-rata " + kota_key):
            for key, val in defaults.items():
                st.session_state[key] = val

    st.divider()

    # Input fitur dengan nilai default dari session state
    col1, col2, col3 = st.columns(3)

    with col1:
        elevation = st.number_input("Elevation (m)",
            value=st.session_state.get("elevation", defaults["elevation"]), step=1.0)
        slope = st.number_input("Slope (°)",
            value=st.session_state.get("slope", defaults["slope"]), step=0.1)
        aspect = st.number_input("Aspect (°)",
            value=st.session_state.get("aspect", defaults["aspect"]), step=1.0)
        TWI = st.number_input("TWI",
            value=st.session_state.get("TWI", defaults["TWI"]), step=0.1)

    with col2:
        HAND = st.number_input("HAND (m)",
            value=st.session_state.get("HAND", defaults["HAND"]), step=0.1)
        NDVI = st.number_input("NDVI",
            value=st.session_state.get("NDVI", defaults["NDVI"]),
            step=0.01, min_value=-1.0, max_value=1.0)
        MNDWI = st.number_input("MNDWI",
            value=st.session_state.get("MNDWI", defaults["MNDWI"]),
            step=0.01, min_value=-1.0, max_value=1.0)

    with col3:
        NDBI = st.number_input("NDBI",
            value=st.session_state.get("NDBI", defaults["NDBI"]),
            step=0.01, min_value=-1.0, max_value=1.0)
        SAR_baseline = st.number_input("SAR VV Baseline (dB)",
            value=st.session_state.get("SAR_baseline", defaults["SAR_baseline"]), step=0.1)
        dist_river = st.number_input("Jarak ke Sungai (m)",
            value=st.session_state.get("dist_river", defaults["dist_river"]), step=10.0)
    ...
    with col_pred:
        # Threshold yang dipakai (dari slider Tab 2)
        if "Bandung" in kota:
            threshold_used = st.session_state.get("slider_bdg", rf_threshold)
        else:
            threshold_used = st.session_state.get("slider_bgr", xgb_threshold)
        ...
        if st.button("🔍 Prediksi Sekarang", type="primary"):
            ...
            if pred == 1:
                st.error("⚠️ **BAHAYA BANJIR TERDETEKSI**")
            else:
            ...
            ax.axvline(x=threshold_used, color='black', linestyle='--',
                       linewidth=2, label=f'Threshold ({threshold_used:.2f})')
            ax.set_xlim(0, 1)
    ...
    with col_map:
        ...
        # Marker lokasi yang diprediksi
        folium.Marker(
            location=center,
            popup=folium.Popup(
                f"<b>{kota_key}</b><br>"
                f"Elevation: {elevation:.0f} m<br>"
                f"Slope: {slope:.1f}°<br>"
                f"dist_river: {dist_river:.0f} m",
                max_width=200
            ),
            icon=folium.Icon(color="red", icon="exclamation", prefix="fa")
        ).add_to(m_pred)

        # Circle radius berdasarkan probabilitas
        folium.Circle(
            location=center,
            radius=500,
...
🌊 Flood Hazard Explorer | Urban Analytics UAS 2025<br>
Kota Bandung (Random Forest) | Kota Bogor (XGBoost)<br>
Data: Sentinel-1/2 · SRTM DEM · GHSL · JRC Water · HydroSHEDS
</div>
